...
pub fn rect(ctx: &mut impl WideRenderContext, x: i32, y: i32, w: i32, h: i32, fill: Fill, z: f32) {
    match fill {
        ...
        Fill::Drawable(drawable, orientation) => {
            ...
            let v1 = shapes::vertex1(x, y, tex.id, uv[0]);
            let v2 = shapes::vertex1(x, y + h, tex.id, uv[1]);
            let v3 = shapes::vertex1(x + w, y + h, tex.id, uv[2]);
            let v4 = shapes::vertex1(x + w, y, tex.id, uv[3]);

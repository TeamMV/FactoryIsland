...
use crate::game::world::tiles::{StaticTile, Tile, TILE_SIZE};
use crate::res::R;
...
impl Layer {
    fn new() -> Self {
        Self {
            tiles: [0; CHUNK_SIZE * CHUNK_SIZE].map(|_| Tile::Air),
        }
    }
...
pub struct Chunk {
    ...
    pub y_levels: [i32; CHUNK_SIZE * CHUNK_SIZE]
}
...
impl Chunk {
    pub fn new(chunk_world_x: i32, chunk_world_z: i32, seed: u32) -> Self {
        Self {
            ...
            air_tile: Tile::Air,
            layers: Vec::new(),
            y_levels: [0; CHUNK_SIZE * CHUNK_SIZE],
        }
    }
        ...
        if let Tile::Air = tile {
            if pos.raw.1 == self.y_levels[tile_idx] {
                for layer_idx in (0..self.layers.len()).rev() {
                    ...
                    if !matches!(layer_tile, Tile::Air) {

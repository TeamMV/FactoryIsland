...
impl Mainscreen {
    pub fn new(window: &Window, game: &Game) -> Self {
        ...
        modify_style!(
            hover_style.background.color = UiValue::Just(parse_color("#3f48cc").unwrap())
        );
...
impl GameUiCallbacks for Mainscreen {
    ...
    fn check_ui_events(&mut self, window: &mut Window, game_handler: &mut GameHandler) {
        if self.connect_btn.was_left_clicked() {
            ...
            let conn = Client::connect(
                ...
                game_handler
                    ...
                    .deref()
                    .upgrade()
                    .expect("This can never be invalid!"),
            );
